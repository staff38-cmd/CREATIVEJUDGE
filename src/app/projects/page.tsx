...
import { Client, Project, WorkSummary } from "@/lib/types";
...
export default function ProjectsPage() {
  ...
  async function load() {
    const [projectsRes, worksRes, clientsRes] = await Promise.all([
      fetch("/api/projects"),
      fetch("/api/works"),
      fetch("/api/clients"),
    ]);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (res.ok) {
        setNewName(""); setNewClientId(""); setNewDescription("");
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400">{filtered.length} 件</p>
        </div>
        <button
      ...
      {clients.length > 0 && (
        <div className="flex gap-2 mb-6 flex-wrap">
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${filterClientId === "" ? "bg-white/20 text-white border-white/40" : "border-white/10 text-gray-400 hover:border-white/30"}`}
          >
            すべて
          </button>
          {clients.map((c) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${filterClientId === c.id ? "bg-blue-500/20 text-blue-300 border-blue-500/40" : "border-white/10 text-gray-400 hover:border-white/30"}`}
            >
              {c.name}
            </button>
          ))}
        ...
        <div className="mb-8 p-6 rounded-2xl border border-violet-500/30 bg-violet-500/5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => { setShowCreateForm(false); setNewName(""); setNewClientId(""); setNewDescription(""); }}
...
function ProjectCard({
  ...
  const displayClient = client?.name ?? project.clientName;
  ...
  return (
    <div className="p-5 rounded-2xl border border-white/10 bg-white/5 hover:border-white/20 transition-all">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 ml-9 mt-2 flex-wrap">
            {(project.companyRegulations || project.client?.companyRegulations) && (
      ...
      <div className="flex items-center gap-3 mt-4 pt-4 border-t border-white/5">
        <Link href={`/works?project=${project.id}`} className="px-4 py-2 rounded-lg text-xs font-medium bg-white/5 hover:bg-white/10 transition-colors">
          チェック履歴 →
        </Link>
        <Link href={`/submit?project=${project.id}`} className="px-4 py-2 rounded-lg text-xs font-medium bg-violet-500/20 text-violet-300 hover:bg-violet-500/30 transition-colors">
          ＋ チェック追加
        </Link>
        <Link href={`/projects/${project.id}`} className="px-4 py-2 rounded-lg text-xs font-medium border border-white/10 hover:border-white/30 hover:bg-white/5 transition-colors">
          ⚙ 設定
        </Link>
        <button
